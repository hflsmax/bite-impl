...
let rec get_fun_info ((exp, ty, effs, attrs): R.expr) (pfun_name : string option) : fun_info list * name list =
  ...
  | If (e1, e2, e3) ->
      get_fun_info e1 pfun_name @++@ get_fun_info e2 pfun_name @++@ get_fun_info e3 pfun_name
  | Let (x, ty, e1, e2) ->
      get_fun_info e1 pfun_name @++@ get_fun_info e2 pfun_name
  | Decl (x, ty, e1, e2) ->
      get_fun_info e1 pfun_name @++@ get_fun_info e2 pfun_name
  | Handle (x, fname, exp_catch, exp_handle) ->
    let[@warning "-partial-match"] (R.FullFun (_, fun_name, _, _, _, _, _, _), _, _, _) = exp_catch in
      ([], [fun_name]) @++@ get_fun_info exp_catch pfun_name @++@ get_fun_info exp_handle pfun_name
  | FullFun (_, fun_name, es1, hs, tm_args, ty, es2, exp_body) ->
    ...
    {fun_name; pfun_name; locals = tm_args @ hd_args @ gather_locals exp_body; handlers} :: fun_infos, []
  | FullApply (exp, es, hs, exps) ->
      get_fun_info exp pfun_name @++@ List.fold_left (fun acc exp_iter -> get_fun_info exp_iter pfun_name @++@ acc) ([], []) exps
  | Raise (h, es, hs, exps) ->
      List.fold_left (fun acc exp_iter -> get_fun_info exp_iter pfun_name @++@ acc) ([], []) exps
...
let get_env_struct {fun_name; pfun_name; locals; handlers} : string =
  ...
  let jbs = List.map (fun h -> spf "jmp_buf %s_jb;\n" h) handlers |> String.concat "" in
  (match pfun_name with None -> "" | Some pfun_name' -> spf "typedef %s_locals_t %s_env_t;\n" pfun_name' fun_name) ^
