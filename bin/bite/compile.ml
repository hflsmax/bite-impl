...
let can_be_returned exp =
  ...
  | Assign _ | If _ | Let _ | Decl _ | Handle _ | FullFun _ | Resume _ | Seq _-> false
...
let rec compile ((exp, ty, effs, attrs) as exp': R.expr) : string * string list =
  begin match exp with
    ...
    | FullApply ((lhs_name, lhs_ty, _, _) as lhs, es, hs, exps) ->
      ...
      let exps', f2 = List.split (List.map (fun ((exp, ty, _, _) as rexp) -> let exp_code, fs = compile rexp in match ty with
  ...
  |> fun (code, fs) -> (if attrs.cfDest = Abort then code ^ "_longjmp(jb, 1);\n" else code), fs