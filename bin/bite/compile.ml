...
let rec compile (cf_dest : cf_dest) ((exp, ty, effs, attrs) as exp': R.expr) : string * string list =
  begin match exp with
    ...
    | FullApply ((_, lhs_ty, _, _) as lhs, es, hs, exps) ->
      ...
      let args_code = if attrs.isRecursiveCall then ("locals.env") :: exps' @ handler_args else (lhs' ^ "_env") :: exps' @ handler_args in
      let lhs_code = if Option.is_some attrs.topLevelFunctionName then Option.get(attrs.topLevelFunctionName) else spf "((%s)%s_fptr);" (tabs_to_string lhs_ty false) lhs' in
