...
let gather_exp predicate exp =
  ...
  | Handle (x, h, exp_catch, exp_handle) -> gather_exp' exp_catch @ gather_exp' exp_handle
  | FullFun (kind, x, es1, hs, tm_args, ty, es2, exp_body) -> []
  | FullApply (exp, es, hs, exps) -> gather_exp' exp @ List.concat (List.map gather_exp' exps)
