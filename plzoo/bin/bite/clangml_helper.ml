...
let field name qual_type =
  Clang.Ast.node (Clang.Ast.Field {qual_type; name; bitwidth = None; init = None; attributes = []})

let record_decl name fields =
  Clang.Ast.node (Clang.Ast.RecordDecl
    { name; fields; complete_definition = true; attributes = []; 
      keyword = Struct; nested_name_specifier = None; 
      bases = []; final = false; is_injected_class_name = false })

let translation_unit decls =
  Clang.Ast.node (Clang.Translation_unit.make decls)
