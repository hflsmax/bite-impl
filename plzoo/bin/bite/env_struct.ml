...
type fun_info = name * name * (name * ty) list
[@@deriving sexp]

let rec get_fun_info ({Zoo.data=exp; loc} : expr) (p_fun : string) : fun_info list  =
  match exp with
  | Var (_, x) -> []
  ...
  | FullFun (x, es1, hs, tm_args, ty, es2, exp_body) ->
    ...
    (x, p_fun, (x, full_fun_to_tabs exp) :: tm_args @ hd_args @ gather_locals exp_body) :: get_fun_info exp_body x
  | FullApply ((exp, _), es, hs, exps) ->
...
let get_env_and_closure_decl (name, p_fun, locals) : string =
  let env_struct_decl = 
    let locals = List.map (fun (x, ty) -> spf "%s %s;\n" (ty_to_string ty) x) locals |> String.concat "" in
    (spf "typedef %s_locals_t %s_env_t;\n" p_fun name) ^
