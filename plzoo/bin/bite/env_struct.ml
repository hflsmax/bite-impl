...
let rec get_env_struct ({Zoo.data=exp; loc} : expr) (p_fun : string option) : env_struct list =
  match exp with
  | Var (_, x) -> []
  ...
  | FullFun (x, es1, hs, tm_args, ty, es2, exp_body) ->
    (x, p_fun, (x, full_fun_to_tabs exp) :: tm_args @ gather_locals exp_body) :: get_env_struct exp_body (Some x)
